...
class _ChatTile extends StatelessWidget{
  ...
  @override Widget build(BuildContext context)=>Material(color:Colors.transparent,child:InkWell(onTap:onTap,borderRadius:BorderRadius.circular(24),child:Ink(padding:const EdgeInsets.all(13),decoration:BoxDecoration(color:data.unread>0?const Color(0xFF171121):const Color(0xFF111019),borderRadius:BorderRadius.circular(24),border:Border.all(color:data.unread>0?const Color(0xFF4A286D):const Color(0xFF292233))),child:Row(children:[
    Stack(children:[Container(width:62,height:62,padding:const EdgeInsets.all(2),decoration:BoxDecoration(shape:BoxShape.circle,border:Border.all(color:data.online?context.tokens.lime:const Color(0xFF544B60),width:2)),child:ClipOval(child:Image.network(data.image,fit:BoxFit.cover,errorBuilder:(_,__,___)=>const ColoredBox(color:Color(0xFF251A35),child:Icon(Icons.person,color:Colors.white70))))),if(data.online)Positioned(right:1,bottom:2,child:Container(width:15,height:15,decoration:BoxDecoration(shape:BoxShape.circle,color:context.tokens.lime,border:Border.all(color:const Color(0xFF111019),width:3))))]),
    const SizedBox(width:13),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[Text(data.name,style:TextStyle(color:Colors.white,fontSize:17,fontWeight:data.unread>0?FontWeight.w900:FontWeight.w700)),if(data.verified)...[const SizedBox(width:5),const Icon(Icons.verified_rounded,color:Color(0xFF8B34FF),size:17)],const Spacer(),Text(data.time,style:TextStyle(color:data.unread>0?context.tokens.lime:const Color(0xFF8E8699),fontSize:12,fontWeight:FontWeight.w700))]),const SizedBox(height:6),Row(children:[Expanded(child:Text(data.message,maxLines:1,overflow:TextOverflow.ellipsis,style:TextStyle(color:data.unread>0?const Color(0xFFECE6F3):const Color(0xFFAAA2B4),fontWeight:data.unread>0?FontWeight.w700:FontWeight.w500))),if(data.unread>0)Container(margin:const EdgeInsets.only(left:8),constraints:const BoxConstraints(minWidth:23,minHeight:23),alignment:Alignment.center,padding:const EdgeInsets.symmetric(horizontal:6),decoration:BoxDecoration(color:context.tokens.lime,borderRadius:BorderRadius.circular(99)),child:Text('${data.unread}',style:const TextStyle(color:Colors.black,fontSize:11,fontWeight:FontWeight.w900)))])]))
  ]))));
}
...
class _ChatScreenState extends State<ChatScreen>{final input=TextEditingController();final List<String> sent=[];@override void dispose(){input.dispose();super.dispose();}@override Widget build(BuildContext context)=>Scaffold(backgroundColor:const Color(0xFF090712),appBar:AppBar(backgroundColor:const Color(0xFF100B18),foregroundColor:Colors.white,titleSpacing:0,title:Row(children:[CircleAvatar(radius:20,backgroundImage:NetworkImage(widget.data.image)),const SizedBox(width:10),Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(widget.data.name,style:const TextStyle(fontSize:16,fontWeight:FontWeight.w900)),Text(widget.data.online?'Şu an aktif':'Yakın zamanda aktif',style:TextStyle(fontSize:11,color:widget.data.online?context.tokens.lime:Colors.white54))])]),actions:[IconButton(onPressed:(){},icon:const Icon(Icons.more_horiz_rounded))]),body:Column(children:[Expanded(child:ListView(padding:const EdgeInsets.all(18),children:[const _TheirBubble('Merhaba 👋 Profilindeki seyahat ilgisini gördüm.'),const _MyBubble('Selam! Evet, yeni yerler keşfetmeyi çok seviyorum.'),_TheirBubble(widget.data.message),...sent.map((e)=>_MyBubble(e))])),SafeArea(top:false,child:Padding(padding:const EdgeInsets.fromLTRB(12,8,12,12),child:Row(children:[IconButton(onPressed:(){},icon:Icon(Icons.add_circle_outline_rounded,color:context.tokens.lime)),Expanded(child:TextField(controller:input,style:const TextStyle(color:Colors.white),decoration:InputDecoration(hintText:'Mesaj yaz...',filled:true,fillColor:const Color(0xFF17131F),border:OutlineInputBorder(borderRadius:BorderRadius.circular(24),borderSide:BorderSide.none))),),const SizedBox(width:8),IconButton.filled(onPressed:(){final t=input.text.trim();if(t.isEmpty)return;setState((){sent.add(t);input.clear();});},style:IconButton.styleFrom(backgroundColor:context.tokens.lime,foregroundColor:Colors.black),icon:const Icon(Icons.send_rounded))])))]));}
