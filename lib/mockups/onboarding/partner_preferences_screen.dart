...
class _PartnerPreferencesScreenState extends State<PartnerPreferencesScreen> {
  ...
  void _pointerMove(PointerMoveEvent e){
    _dragDistance+=e.delta.dy;
    if(_dragDistance < -70 && _heroVisible){_dragDistance=0;setState(()=>_heroVisible=false);}
    else if(_dragDistance > 70 && !_heroVisible){_dragDistance=0;setState(()=>_heroVisible=true);}
  }
  ...
  Widget build(BuildContext context){
    ...
    return Scaffold(backgroundColor:black,body:SafeArea(child:Listener(
      behavior:HitTestBehavior.translucent,onPointerMove:_pointerMove,onPointerUp:_pointerEnd,onPointerCancel:_pointerEnd,
      child:Column(children:[
        AnimatedSize(duration:const Duration(milliseconds:700),curve:Curves.easeInOutCubic,alignment:Alignment.topCenter,child:SizedBox(
          height:_heroVisible?350:0,width:double.infinity,child:ClipRect(child:AnimatedSlide(
            duration:const Duration(milliseconds:660),curve:Curves.easeInOutCubic,offset:_heroVisible?Offset.zero:const Offset(0,-1.05),
            child:AnimatedOpacity(duration:const Duration(milliseconds:500),curve:Curves.easeInOut,opacity:_heroVisible?1:0,child:_PartnerHero(onBack:()=>Navigator.of(context).pop(),selectedCount:selectedPreferenceCount,remainingRights:remainingRights)),
          )),
        )),
        Expanded(child:ListView.separated(
          physics:const AlwaysScrollableScrollPhysics(),padding:EdgeInsets.fromLTRB(context.tokens.spaceLg,_heroVisible?14:8,context.tokens.spaceLg,40),itemCount:questions.length,
          separatorBuilder:(_,__)=>const SizedBox(height:12),itemBuilder:(context,i){
            final values=selections[i]??<String>{};final locked=limitReached&&values.isEmpty;
            return Column(children:[
              if(i==0&&!_heroVisible)Align(alignment:Alignment.centerLeft,child:IconButton.filledTonal(onPressed:()=>Navigator.of(context).pop(),icon:const Icon(Icons.arrow_back_rounded))),
              _PreferenceTile(question:questions[i],selectedValues:values,locked:locked,onTap:locked?null:()=>_openQuestion(i)),
            ]);
          },
        )),
      ]),
...
class _PartnerPreferenceDetailScreenState extends State<PartnerPreferenceDetailScreen>{
  late Set<String> selected;bool doesntMatter=false;bool _heroVisible=true;double _dragDistance=0;
  ...
  void _pointerMove(PointerMoveEvent e){_dragDistance+=e.delta.dy;if(_dragDistance < -60&&_heroVisible){_dragDistance=0;setState(()=>_heroVisible=false);}else if(_dragDistance > 60&&!_heroVisible){_dragDistance=0;setState(()=>_heroVisible=true);}}
  void _pointerEnd(PointerEvent e)=>_dragDistance=0;
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(backgroundColor:black,body:SafeArea(child:Listener(
      behavior:HitTestBehavior.translucent,onPointerMove:_pointerMove,onPointerUp:_pointerEnd,onPointerCancel:_pointerEnd,
      child:Column(children:[
        AnimatedSize(duration:const Duration(milliseconds:650),curve:Curves.easeInOutCubic,alignment:Alignment.topCenter,child:SizedBox(
          height:_heroVisible?210:0,width:double.infinity,child:ClipRect(child:AnimatedSlide(duration:const Duration(milliseconds:620),curve:Curves.easeInOutCubic,offset:_heroVisible?Offset.zero:const Offset(0,-1.08),child:AnimatedOpacity(duration:const Duration(milliseconds:480),curve:Curves.easeInOut,opacity:_heroVisible?1:0,child:_PreferenceDetailHero(icon:widget.question.icon,onBack:()=>Navigator.of(context).pop())))),
        )),
        Expanded(child:SingleChildScrollView(physics:const AlwaysScrollableScrollPhysics(),padding:EdgeInsets.fromLTRB(context.tokens.spaceLg,14,context.tokens.spaceLg,120),child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
...
class _PartnerHero extends StatelessWidget{
  const _PartnerHero({required this.onBack,required this.selectedCount,required this.remainingRights});final VoidCallback onBack;final int selectedCount,remainingRights;
  @override Widget build(BuildContext context)=>SizedBox(height:350,width:double.infinity,child:ClipPath(clipper:_OvalBottomClipper(),child:Container(
    ...
    child:Stack(children:[
      ...
      Positioned(left:28,right:28,top:120,child:Container(padding:const EdgeInsets.all(18),decoration:BoxDecoration(color:Colors.white.withOpacity(.08),borderRadius:BorderRadius.circular(24),border:Border.all(color:Colors.white.withOpacity(.16))),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        ...
        const SizedBox(height:16),Text('2 başlangıç tercihinden $selectedCount tanesi seçildi',style:context.texts.titleSmall?.copyWith(color:Colors.white,fontWeight:FontWeight.w800)),const SizedBox(height:10),
        ClipRRect(borderRadius:BorderRadius.circular(999),child:LinearProgressIndicator(value:selectedCount/2,minHeight:7,backgroundColor:Colors.white.withOpacity(.18),valueColor:AlwaysStoppedAnimation<Color>(context.tokens.lime))),const SizedBox(height:14),
...
class _PreferenceDetailHero extends StatelessWidget{
  const _PreferenceDetailHero({required this.icon,required this.onBack});final IconData icon;final VoidCallback onBack;
  @override Widget build(BuildContext context)=>SizedBox(height:210,width:double.infinity,child:ClipPath(clipper:_OvalBottomClipper(),child:Container(
    decoration:const BoxDecoration(gradient:LinearGradient(begin:Alignment.topLeft,end:Alignment.bottomRight,colors:[Color(0xFF9A4DFF),Color(0xFF6D21D7),Color(0xFF45108D)])),
    child:Stack(children:[Positioned(left:16,top:14,child:IconButton.filledTonal(onPressed:onBack,icon:const Icon(Icons.arrow_back_rounded))),Positioned(left:28,top:86,child:Text('Partner Tercihleri',style:context.texts.headlineSmall?.copyWith(color:Colors.white,fontWeight:FontWeight.w900))),Positioned(right:34,top:40,child:_HeroIcon(icon:icon))]),
  )));
}
